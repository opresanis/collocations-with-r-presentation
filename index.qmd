...
![](figures/t_values_simulation.png){width="130%"}
...
In the last meeting on 6 May 2024, synthetic sample of frequency data was constructed to simulate the t-score:

- corpus size 1M tokens
- a simple random sample of 
    * 100 collocation values between 1 and 5000
    * node and collocate frequency between 1 and 5000 but strictly larger than its corresponding collocation value


The plot shows the correlation between the collocation frequency and the t-score. We observe a curvilinear relationship.


It can be concluded that the t-score foremost measures collocation frequency rather than association strength.
...
## Interpretation of AM Distributions
...
Each association measure transforms the normally distributed co-occurrence sampling in a distinct way:

- except of _Log Likelihood_ all measures compress the interquartile range, emphasizing the tails of the distributions (visible as outliers) 
- _MU_ and _MI_ appear like each other's invert (but they are not)
- The increasing exponents of the numerators in _MI_, _MI~2~_ and _MI~3~_ pushes the IQR to the top
- _Log Dice_ and _MI~3~_produce a very similar distribution
- _Log Ratio_ keeps the median centered, with almost symmetrical appearance of both ends
- _z-score_ and _Log Dice_ both have one prominent outlier
...
## Interpretation of Value Distributions {visibility="hidden"}
...
A random sample of 150 common nouns from the BNC 1994, stratified by 10 frequency bands. $\approx 250,000$ node-collocate pairs.  

\

Since we draw from a Zipf-like distribution, the observed frequency effect is less pronounced, but still visible for some measures (notably _t-score_ and _MI~3~_).
...
Using a log-scale for the x-axis gives further insights:

- _MI~3~_ almost 'guarantees' a high score for high cooccurence frequency
- many measures show a _lower bound_ with increasing frequency
- others, notably _MI_ and _Log Ratio_ converge
:::
...
*   We see a clear distinction between measures on a standard vs a log scale (with the exception of Log Likelihood)
*   standard scale: _t-score_ 'centralizes' the distribution, whereas _MU_ shows a similar appearance than the observed frequency
* Again, _z-score_ and _Dice_ look very similar.
* log-scaled measures have their interquartile range below the 0.5 mark, resulting in many outliers in the upper scale
*   _Log Dice_ is the only measure which produces a significant number of outliers on both ends 
:::
...
- Node, collocate and collocation frequency
- Axis are log-scaled
- Orange: top 10 function words in the corpus ('the', 'of', 'and',...)
\
- high collocation frequency is dominated by function words 
- the 'voids' in the node/collocate scatters are due to the sampling process
...
## Summary: the Algorithmic Approach {visibility="hidden"}
...
A good starting point to dive deeper into statistics and R for Corpus linguistics is @Brezina2018. This book gives a solid overview about statistical concepts used in the field. Especially on Collocations, see also @Brezina2015.

Also, the work of Stefan Gries in recommended, in particular @Gries2016, which is a good hands-on introduction into R and its application to case studies. A broader focus on linear and mixed models has @Gries2021. Both give a solid and ridgid introduction into working with R in (Corpus) Linguistics. 

The _Practical Handbook of Corpus Linguistics_ [@Paquot2020] offers an overview on many aspects in Corpus Linguistics research, from corpus compilation to writing a research report. Part II focuses on Corpus methods (with Chapter 7 on _Co-occurrence Data_, mainly Collocations [@Gries2020]) and Part IV on technical aspects, like programming with R.

There are many other introductions to be considered, for example Bodo Winter -@Winter2019. He gives an introduction based on the `tidyverse` dialect of R (other than Gries, who sticks to `base R`) before diving into linear models and inferrential statistics. Slightly earlier work of Guillaume Desagulier -@Desagulier2017 but likewise a good introduction to the topic. Please see @Kortmann2021 for further readings.
